...
Itt van három **szabály-dokumentum** is, nem mérés: a `summary-voice.md` (hogyan
...
vonatkoznak. A harmadik a `kele-janek.md`: a heti rovat hangja, és az, mit nem csinálhat — nem párharchoz tartozik, hanem a fordulóhoz.
...
| `draft-preview-harvest.py` | a következő Draft-forduló **beharangozó-rétege** a saját adatunkból | nem azt nézi, mi történt, hanem ami a jövő heti eredményt befolyásolja: hányszor kellett a gépnek beugrania, hány pont maradt kint a padon, mennyit hoztak a keretmozgások, mennyi ballaszt van a kezdőben, és mennyire függ a csapat egyetlen embertől |
| `mlsz-arelozmeny.txt` + `mlsz-arelozmeny-meres.py` | ad-e a játékos-adatlap fordulónkénti árat, és melyik mezőből jön a forduló | a listás törzs csak a mostani árat adja, a saját árnaplónk pedig csak a saját indulása óta gyűlik; a sor `round_id`-je a forduló azonosítója (75 + 2 × forduló), nem a sor `id`-je — a kettő összekeverése csendben rossz fordulóba tenné az árakat |
| `kele-hang-elemzes.py` | a letöltött cikkekből **megszámolja**, melyik fogás hány cikkben tér vissza — hálózat nélkül, a `tartalek/kele/` alól | egy fogás akkor tartozik a szerző hangjához, ha több cikkben ott áll; egyetlen cikk formája az adott cikké. A mérés kimutatta, hogy a kézzel beillesztett Substack-esszék fogásai (önkorrekció a második mondatban, „Félreértés ne essék", hármas engedmény, számozott tézisek) a futballos írásaiban **egyszer sem** fordulnak elő — azok a hírlevele hangjai, nem a futballrovatáé |
| `kele-hang-meres.py` | az utánzott hang forrása: a szerző saját futballos szövegei, a `tartalek/kele/` alá mentve | a rovat egy létező publicista hangját imitálja, azt pedig olvasni kell, nem találgatni. A szerzői oldalon ott áll a lap „legfrissebb hírek" sávja is, tele idegen cikkel: az első futás emiatt időjárás-előrejelzést és bűnügyi híreket hozott le úgy, hogy közben sikeresnek látszott. Ezért minden letöltött lapnál a **byline** dönt, nem a hivatkozás helye, és a végén kiírja, hányat dobott el. A szöveg a repóba kerül — futási naplóból pár nap múlva már nem lehet visszaolvasni |
| `karszalag-harvest.py` | az NB1 kapitányi rétege több fordulón át: ki kire tette a szalagot, mikor gondolta meg magát, és mit hozott a saját keretének legjobbjához képest | egy forduló kapitányi listája semmit nem mond; négy egymás utáni forduló viszont megmutatja, dönt-e valaki vagy csak ragaszkodik — az 5–8. fordulóban huszonnégy újragondolási alkalomból öt volt a változtatás. A tárolt `week` a kapitánynál már duplázva, a padon ülőnél már felezve áll, tehát vissza kell osztani |
| `kispad-harvest.py` | a Draft kispadjának rétege: a padon maradt termés, a tökéletes kezdővel nyerhető többlet, és azok a vereségek, ahol a pad önmagában több volt, mint a különbség | a három szám nem ugyanaz, és könnyű összekeverni: a padon maradt 272 felső korlát (négy embert tizenegy helyre nem lehet beállítani), a valódi kár a párosítással számolt +209 |
| `idovonal.py` | a forduló eseményei EGY órán, párharconként | a kezdési időhöz adott gólperc megmutatja, hol fordult meg egy párharc, és mi történt közben egy másik pályán — az 5. fordulóban Isak és Semenyo gólja ugyanabban a percben esett két külön meccsen |
| `colour-check.py` | a forduló színestáblájának ellenőrzése — hálózat nélkül, a saját adatunkból | minden `event` sor megtalálja a gólt, gólpasszt, VAR-döntést vagy nagy helyzetet ugyanannak a fájlnak a strukturált blokkjában, és minden játékos benne van valakinek a keretében; e nélkül a mérés eredménye csak emlékezetben él |
| `pl-textstream.py` | a Premier League **saját perces kommentárja** egy fordulóra (pulselive), a mieinkhez párosítva | ez a PL prózai forrása — a FotMob csak strukturált adatot ad; a szöveg csak a futás naplójába kerül, a repóba nem |
