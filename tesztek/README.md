...
| fájl | mit ellenőriz |
|---|---|
| `magazin.teszt.js` | A **Nemzethy Sport** oldal. A lényege a **kapu**: a magazin nem mutathat többet, mint a meccs adatlapja, ezért a teszt kiszámolja az adatból, melyik forduló végleges, és megköveteli, hogy le nem zárt forduló összefoglalója itt se legyen kint. Ezen felül: a fejlécben a lap neve és a szlogen; minden írás **teljes szöveggel** (a bekezdésszámot a fájlból ellenőrzi, nem bedrótozva); a **négy legördülő szűrő** (liga, szakvezető, forduló, típus), az összjátékuk — szakvezetőre szűrve minden írás róla szól, és a fordulólista is vele szűkül —, hogy a szűrősáv nem eszi meg a lapot, és hogy a törlés visszaadja az egészet; az értékelő sáv minden cikk alatt; a „Másolom” a rövid változatot és a cikkre mutató hivatkozást adja; és hogy a láblécből a liga-oldalról is elérhető. Külön méri a **liga-üzemmódot** (`?liga=`): a lap átveszi a liga színeit és nevét, csak az ő írásait mutatja, szűrő és liga-cím nélkül, telefonon nem lóg ki oldalra — és a **belépési pontokat**: a liga-oldal felső sávjából a saját újságja nyílik, a kezdőlapról a közös, az újság saját lapján pedig nincs önmagára mutató gomb. |
| `cikkertekeles.teszt.js` | Az írás értékelése a cikksáv alján. Azt méri, ami a funkció célja: nem a pontszámot, hanem az **indokot**. Rögzíti, hogy a sáv csak a kinyitott cikkben van ott; hogy a hármas-négyes egyetlen koppintással, kérdés nélkül megy el; hogy az egyes-kettes **helyben** nyit indok-panelt, és a cikk szövege pixelre a helyén marad (nem takarjuk el, amiről véleményt kérünk); hogy nincs készre választható ok, és az „Elküldöm” üres dobozzal nem kínálja magát, a kiút viszont végig elérhető; hogy a kiút elküldi a pontot, de **indok nélkül** — amit a néző beírt, azt nem írjuk mellé; hogy a kattintás nem csukja be a cikket; és hogy sikertelen küldésnél nem vész el, amit beírtak. |
| `ertekeles.teszt.js` | A Worker értékelés-tárolója (`/ertekeles`), hamis KV-vel — a Workert innen nem lehet elérni. Rögzíti, hogy a pont és a néző saját szavai együtt tárolódnak, kész kategóriák viszont nem — azokat a Worker el sem tárolja; hogy ugyanaz az eszköz **felülír, nem halmoz**, más eszköz viszont külön sort kap; hogy a tartományon kívüli pont, az ismeretlen cikk-azonosító és a rossz eszközazonosító 400-at kap és **nem ír**; hogy a túl hosszú indok le van vágva; és hogy KV-kötés nélkül a Worker megmondja, hogy nem ment el, nem hazudik nyugtát. |
| `articlestrip.teszt.js` | A beharangozó / összefoglaló sávja a meccs adatlapján. A sáv a keret-oszlopok FÖLÖTT ül, ezért minden állítás arról szól, hogy nem áll az adat útjába: csukva jön, egyetlen sor magas (a felütés bármilyen hosszan is egy sorban fut, a vége levágva), a keret-oszlopok a képernyőn maradnak gépen és mobilon is, becsukás után pedig pontosan visszaállnak. Rögzíti azt is, hogy a szöveg bekezdésről bekezdésre az `articles.json`-é, hogy az újrarajzolás nem csukja be a néző alatt (élő fordulón a test percenként újraépül), hogy cikk nélküli meccsen nyoma sincs a sávnak — és hogy keret nélküli fordulón a beharangozó **nyitva** jön, de a néző saját kattintása azt is felülírja. Külön méri a **zárás-kaput** mindkét ligán: le nem zárt fordulón ugyanarra a meccsre csak a beharangozó jön elő, az összefoglaló nem — `?draft=1`-gyel viszont igen, vázlat-jelzéssel. Külön méri a **zárás-kaput** mindkét ligán, és — ez a legfontosabb állítása — hogy `?draft=1` **nélkül** egyetlen sáv sem jelenik meg: végigmegy az összes olyan meccsen, amihez van vázlat-szöveg, és azt követeli, hogy nyoma se legyen. |
...
| `verzio.teszt.js` | **Az elavult lap magától újratölt.** A `?v=` a `funtasy.js`/`css` gyorsítótárát töri, a lap saját HTML-jét nem — pedig abban éles logika él, és egy régi HTML a régi szabály szerint fut. Azonos verziónál nincs újratöltés, újabbnál pontosan egy (mindkét ligán), és a `verzio.json` a futó lap `?v=` számával egyezik. |
...
| `dokuk.py` | A dokumentáció konzisztenciája: minden tesztfájlnak és gyökérbeli adatfájlnak van sora a README-kben, a fájl-táblázat csak létező fájlra hivatkozik, a változásnapló bejegyzései teljesek. Azért létezik, mert a doksi-szerkesztés kétszer hasalt el csendben — a kézi ellenőrzés terhelés alatt csúszik el, ez kényszeríti ki. Ellenőrzi azt is, hogy minden oldal ugyanazt a `?v=` gyorsítótár-verziót hivatkozza (ha csak az egyiken emelkedik, a másikon régi `funtasy.js`/`css` marad), és hogy a README tartalomjegyzéke egyezik a tényleges címekkel — a jegyzék nem kézi munka, `python3 tesztek/dokuk.py --javit` újraírja.  **D8**: amit a gyűjtő kiír (`kompakt_iras` / `kiir_ha_valtozott`), az szerepel a workflow `git add` listáján is — a `keretvaltozasok.json` egyszer így maradt ki, és a repóba soha nem került be. És **D9**: a megjegyzések meg a fejlesztői dokumentáció a szabályt mondják el, nem a fejlesztés történetét — nem állhat bennük dátum, döntés-kontextusú név, sem „bejelentett / megtörtént” jellegű napló-jelző. Csak megjegyzés-tartományokat néz, tehát a kódban és az adatban álló valódi dátum (teszt-fixtúra, changelog, `compatibility_date`) érintetlen marad. |
...
| `gyujto_meccsek.py` | A gyűjtő meccsgyűjtése (`meccsek.json`): id szerinti összevonás, **eredmény csak lezárt meccsről** (a futó meccs pontszámát akkor sem tárolja, ha az API küldi), a visszaeső meccs kimarad, a teljes forduló nem kér többé meccslistát, a futó meccsű igen — és a **pótolt meccs** is bekerül: ha a forduló hivatalos pontja változik, újra megy a meccslistás lekérés (az elhalasztott meccs nincs benne a listában, tehát „befejezetlenként” nem látszana).  **M9–M11**: a *pótolt* (korábbi fordulós, most játszott) meccs beleszámít a fordulóba, a klub nem kap „nincs meccse” jelölést, a `start` a legközelebbi le nem ment meccsre mutat, és `vege` csak akkor lesz, ha mindegyik lement — a *visszaeső* meccs viszont továbbra is kiesik. **M11**: ha épp lekértük a meccslistát, a régi `nogame` nem ragad be — enélkül a javítás önmagát blokkolta volna. |
...
| `forduloelott.teszt.js` | A leadási határidő után, de az első sípszó előtt is látszik **mindkét keret** (pont helyett kötőjellel), **„élő” jelzéssel** — a forduló ilyenkor már él, és a fejlécben szám áll, nem 0:0, mert a magyarszabály már pontot ér —, KEZD% sor nélkül — jövőbeli fordulónál viszont marad az üzenet, mert oda nincs mentett keret. Külön, **érintetlen lapon** azt is méri, hogy a főoldali „Következő forduló” listában szám áll és „élő” jelölés van, nem `— : —`. Nem a naptárra támaszkodik: a menetrendből menet közben kiveszi egy lezárt forduló eredményét, és a tárolt keretet is a sípszó előtti állapotra állítja. |
| `keretpotlas.teszt.js` | **A folyó forduló kereteit a böngésző pótolja, amíg a gyűjtő még nem írta be.** Az előfeltételt kimondja: menet közben kiveszi a menetrendből a forduló eredményét, a keret-előzményből pedig magát a fordulót — ez a vizsgált helyzet (fut a forduló, de nincs hozzá tárolt keret). Méri, hogy mind a 10 keret megjön, lesz élő meccsállás és „élő” státusz. Külön méri a **részleges** pótlást: ha egy csapat lekérése elhasal, az ő meccse élő állás nélkül marad (a 0 hamis szám lenne), a többi viszont megkapja. |
| `parhuzamos.teszt.js` | **Az NB1 élő lekérése két körben megy, nem nyolcban.** A szakvezetőnkénti MLSZ-kéréseket mesterséges késleltetéssel méri: az első kérés befejeződik, mielőtt a második elindul (a lekérő ebből tanulja meg a működő proxy-utat), a maradék hét viszont egyszerre indul, és a teljes idő két kör, nem nyolc. Azt is méri, hogy mind a nyolc szakvezető adata megjön, és hogy aki a párhuzamos körből kiesik, azt pontosan egyszer kérjük újra — az ő állása is bekerül. |
| `workertarolo.teszt.js` | **A Worker „utolsó ismert állás" tárolója**, hamis KV-vel és hamis hálózattal (a Workert a fejlesztői környezetből nem lehet elérni). Méri, hogy a ritkán változó válasz átmegy és eltárolódik; hogy **változatlan** válaszra nem ír újra — ez a napi 1000 KV-írásos keret miatt kritikus —, változásra viszont igen; hogy a gyorsítótár-törő `fpl_` bélyeg nem szór szét kulcsokat; hogy a percenként változó PL élő pontot nem tárolja; hogy a `/tarolt` végpont egyetlen kérést sem indít felfelé; hogy KV-kötés nélkül minden marad a régiben; és hogy a `/tarolt` sem szolgál ki idegen eredetet. |
| `taroltallas.teszt.js` | **A tárolt állás már az első képen ott van** (NB1). Lassú élő lekérést szimulál, és méri, hogy a tárolt állás **azelőtt** kirajzolódik, hogy bármelyik élő válasz beérne — egyetlen kör-útból. Azt is nézi, hogy a státuszsáv a **lekérés** idejét írja ki, nem a mostanit, hogy az élő lekérés utána felülírja a tároltat, és hogy üres tárolónál semmi nem romlik el. A LIVE réteget vizsgálja, nem a kiírt szöveget: a szöveges illesztés hamisan talált (a „88" benne van a „71,88" archív eredményben is). |
...
| `kulonbseg.teszt.js` | A Különbségek nézet szerkezete gépen (két oszlop) és mobilon (közös blokk + két oszlop).  Azt is méri, hogy a **közös játékosok vegyes forrásnál** is megvannak: az élő nézet a két keretet külön kéri le, és ha csak az egyik frissül, a két oldal névalakja eltér — név szerint párosítva nulla közös jött ki. Az azonosító párosít. |
| `lekero.teszt.js` | A lekérő tartalék-útjai (a corsproxy.io 401-re váltott, az allorigins túlterhelt volt — minden élő lekérés leállt mindkét ligában). Rögzíti, hogy a lánc végigesik a következő útig (corsproxy 401 + néma allorigins mellett a cors.sh szolgál ki), a csomagolt allorigins-válasz (`{contents:"…"}`) kibontva jelenik meg, és ha minden út elhasal, a hibaüzenet felsorolja őket. |
