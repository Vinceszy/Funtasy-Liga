...
| `gyujto_zarasnb1.py` | A meccs utáni pontigazítás naplózása (`collect.py` `zaras_valtozas`): csak lement meccsnél számít változásnak, a kapitányi duplázás és a padfelezés vissza van számolva (negyedre kerekítve), ugyanaz a változás több keretből egyszer kerül be, de a későbbi újabb igazítást nem nyeli el a dedup, és hiányzó adatból nem talál ki változást. |
| `zarasnb1.teszt.js` | A „Meccs utáni pontigazítás" panel (NB1): üres vagy hiányzó adatnál rejtve marad, alapból a legutolsó ilyen fordulót mutatja, a régi/új érték és az előjeles különbség is látszik (csökkenés és növekedés külön színnel), lapozni lehet, és a csapatszintű sor a szakvezető nevét mutatja, a keretét nyitva. |
